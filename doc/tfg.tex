...
\lstset{
  ...
  keepspaces=true,
  morekeywords={map,eqn,sort,struct,init,delay,yaled,var,proc,act,comm,allow,hide,tau,delta}
  numbers=left,
  numbersep=5pt,
...
\epigraph{To my family and friends.

 No deadlock was found in the production of this work.}
...
\begin{figure}[H]
	\minipage{0.5\textwidth}
	\centering
	\scalebox{.5}{\input{img/CommunicationOperator/Process}}
	\caption{LTS of $ P \Vert Q $} \label{fig:CommunicationOperatorProcess}
	\endminipage\hfill
	\minipage{0.5\textwidth}
	\centering
	\scalebox{.5}{\input{img/CommunicationOperator/Communicate}}
...
\begin{table}[h]
	...
	\begin{tabular}{ c l }
		...
		\infer{\Gamma \vdash p_1\cdot p_2 = q_1\cdot q_2}{\Gamma \vdash p_1 = q_1 & \Gamma \vdash p_2 = q_2} & congruence\\
		\infer{\Gamma \vdash p = q}{\Gamma \vdash p\cdot x = q\cdot x} & extensionality\\	
...
For logical formulas, rules are very straightforward. The only caveat to notice is that we assume that previously we transform every logical statement to compositions of $ (true = false) $ and $ \Rightarrow $ (which is a functionally complete set for propositional logic, see \cite{Vaughan1942-VAURWW}), and $ \forall $.
...
\begin{table}
	...
	\end{tabular}
	\caption{Derivation rules for $ \mu $-calculus formulas}
\end{table}
...
\nextgroupplot[title=Before dpbb,ymode=log,xtick={2,3,4,5},nodes near coords,ylabel=States,xlabel=Threads]
\addplot [mark=*,blue,smooth,point meta = explicit symbolic] coordinates {
      (2,195) [195]
      (3,6922) [6922]
      (4,381949) [381949]
      (5,30689306) [30689306]
  };
\nextgroupplot[title=After dpbb,xtick={2,3,4,5},nodes near coords,ylabel=States,xlabel=Threads]
  \addplot [mark=*,red, smooth,point meta = explicit symbolic] coordinates {
      (2,13) [13]
      (3,40) [40]
      (4,145) [145]
  };
